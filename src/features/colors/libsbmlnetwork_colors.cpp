...
const std::string getHexColorCodeFromHtmlColorName(const std::string& htmlColorName) {
    ...
    if (isTransparentColorName(htmlColorName)) {
        if (colors->find(removeTransparencyFromColorName(htmlColorName))->second.empty())
            return colors->find("transparent")->second;

        return addTransparencyToColorValue(colors->find(removeTransparencyFromColorName(htmlColorName))->second);
    }
...
const bool isTransparentColorName(const std::string& colorName) {
    if (colorName.rfind("_transparent") != std::string::npos)
        return true;
...
const bool isTransparentColorValue(const std::string& colorValue) {
    if (colorValue.size() == 9 && colorValue.at(0) == '#' && colorValue.substr(7, 2) == "00") {
        return true;
    }
...
const std::string addTransparencyToColorName(const std::string& colorName) {
    if (isTransparentColorName(colorName))
        return colorName;

    return colorName + "_transparent";
}

const std::string addTransparencyToColorValue(const std::string& colorValue) {
    if (isTransparentColorValue(colorValue))
        return colorValue;
    else if (isValidHexColorCode(colorValue) && colorValue.size() == 7)
...
const std::string removeTransparencyFromColorName(const std::string& colorName) {
    ...
    return colorName.substr(0, colorName.size() - 12);
}
